...
void Client::init() {
	...
	cout << "Please enter the fileName of the contract you would like to send: ";
	cin >> fileName;
	...
	response = runClient();
	if (response == 0) {
		cout << "Comunication with contract server was successful. Check the log to see what server sent back" << endl;
	}
	else {
		cout << "Something went wrong in the communication with server, check log to determine where the issue occured" << endl;
	}
}
...
int Client::runClient() {
    ...
    // Send the message
    response = sendto(sockfd, message, strlen(message) + 1, 0, (struct sockaddr*)NULL, sizeof(servaddr));
    if (response == -1) {
